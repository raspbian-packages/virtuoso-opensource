...
sticker_init() {
  ...
  LOG "AddressBook VAD Sticker $STICKER creation..."
  ...
  echo "  <sql purpose=\"post-install\">" >> $STICKER
  echo "    <![CDATA[" >> $STICKER
  echo "      registry_set('ab_path', '"$BASE_PATH_CODE"/AddressBook/');" >> $STICKER
  echo "      registry_set('ab_version', '$VERSION');" >> $STICKER
  echo "      registry_set('ab_build', '$PACKDATE');" >> $STICKER
  echo "      DB.DBA.VAD_LOAD_SQL_FILE('"$BASE_PATH_CODE"/AddressBook/sql/ab-a-wa.sql', 1, 'report', $ISDAV);" >> $STICKER
  ...
  echo "    <![CDATA[" >> $STICKER
  echo "      DB.DBA.VAD_LOAD_SQL_FILE('"$BASE_PATH_CODE"/AddressBook/sql/ab-d.sql', 1, 'report', $ISDAV);" >> $STICKER
  echo "    ]]>" >> $STICKER
  echo "  </sql>" >> $STICKER
  echo "</ddls>" >> $STICKER
