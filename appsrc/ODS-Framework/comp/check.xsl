...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:v="http://www.openlinksw.com/vspx/" xmlns:vm="http://www.openlinksw.com/vspx/ods/">
    ...
    <xsl:template match="text()">
      <xsl:variable name="tnode" select="document-literal (., '', 2, 'UTF-8')" />
      <xsl:apply-templates select="$tnode/*|$tnode/processing-instruction ()"/>
      <xsl:apply-templates select="$tnode/processing-instruction ()"/>
    </xsl:template>
