...
--DB.DBA.vhost_remove (lpath=>'/SecRMLoan');
--DB.DBA.vhost_define (lpath=>'/SecRMLoan',ppath=>'/SOAP/', soap_user=>'LOAN3');

DB.DBA.vhost_remove (lpath=>'/SecRMLoan');
DB.DBA.vhost_define (lpath=>'/SecRMLoan',ppath=>'/SecRMLoan/', vsp_user=>'dba');
...
create procedure SECRMLF_DEPLOY ()
{
  ...
  if (exists (select 1 from BPEL..script where bs_name = 'SecRMLoanFlow'))
    return;
  BPEL.BPEL.import_script (sprintf ('http://localhost:%s/BPELDemo/SecRMLoan/bpel.xml', server_http_port ()),
      'SecRMLoanFlow', scp);
