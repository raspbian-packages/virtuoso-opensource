...
--  
--  
create procedure "XQ"."XQ"."DESK_RUN" (in _text varchar, in _compiletime integer, in _expected_text varchar, in _brief integer)
{
  declare _ms1, _ms2, _ctr, _len integer;
  declare _arg, _res, _res_ses any;
  declare _sub_text, _actual_text varchar;
--  _arg := xper_doc('<blank/>', 0, concat('http://localhost:', "XQ"."XQ"."EXTRACT_PORT_FROM_INI" (), '/DAV/xqdemo/nosuchfile.xml'));
  _arg := xper_doc('<blank/>', 0, 'virt://XQ.XQ.TEST_FILES.NAME.TEXT:xqdemo/nosuchfile.xml');
  _ms1 := msec_time();
  _res := xquery_eval(_text,_arg,0);
  _ms2 := msec_time();
...
create procedure "XQ"."XQ"."INDENT_XML" (in _xml any) returns varchar
{
...
create function "XQ"."XQ"."XPER_TO_XTREE" (in _xml any) returns any
{
  if (isstring (_xml) or not isarray(_xml))
    {
      declare _ses any;
  ...
  else
    {
      ...
      while (_ctr < _len)
	{
	  aset (_res, _ctr, "XQ"."XQ"."XPER_TO_XTREE" (aref (_xml, _ctr)));
	  _ctr := _ctr + 1;
	}
      return _res;
    }
