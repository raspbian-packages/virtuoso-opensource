  ...
  <HTML>
    ...
    <BODY>
     ...
     <H4>Results of searching on <xsl:value-of select="$mask" /></H4>
      ...
      <tr>
         <td><xsl:value-of select="@EENAME"/></td>
         <td><xsl:value-of select="@EJOB"/></td>
         <td><xsl:value-of select="@DDNAME"/></td>
         <td><xsl:value-of select="@DLOC"/></td>
      </tr>
      </xsl:for-each>
      </table>
     <p><a href="so_s_14_ora_client.vsp">New search</a></p>
    </BODY>
  </HTML>
</xsl:template>
</xsl:stylesheet>
